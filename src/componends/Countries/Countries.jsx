...
const Countries = ({ countriesPromise }) => {
  ...
  const getCountryCode = (country) =>
    country?.cca3?.cca3 || country?.cca3 || country?.name?.common;
  ...
  const regions = useMemo(() => {
    const uniqueRegions = [
      ...new Set(
        countries
          .map((country) => country?.region?.region || country?.region)
          .filter(Boolean),
      ),
    ];
  ...
  const visitedCountries = useMemo(
    () =>
      countries.filter((country) =>
        visitedCodes.includes(getCountryCode(country)),
      ),
  ...
  const filteredCountries = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      if (sortBy === "population-desc") {
        return (
          (b?.population?.population || b?.population || 0) -
          (a?.population?.population || a?.population || 0)
        );
      }
      if (sortBy === "area-desc") {
        return (
          (b?.area?.area || b?.area || 0) - (a?.area?.area || a?.area || 0)
        );
      }
    ...
    <section className="countries-page">
      <header className="hero">
        ...
        <p className="hero-copy">
          Search, filter, and sort nations instantly. Mark where you have been
          and build your travel memory wall.
        </p>
      </header>
        ...
        <aside className="visited-sidebar">
          <div className="quick-stats">
            ...
            <p className="stat-line">
              Total countries: <strong>{countries.length}</strong>
            </p>
            <p className="stat-line">
              Results shown: <strong>{filteredCountries.length}</strong>
            </p>
            <p className="stat-line">
              Visited: <strong>{visitedCountries.length}</strong>
            </p>
          </div>
